...
function TerminalCard({ id, title, children, pinned, onTogglePin, expandedId, setExpandedId }) {
  ...
  return (
    ...
    className={`relative rounded-3xl border border-emerald-900/40 bg-[#0f1413] shadow-2xl shadow-black/40 transition-all duration-500 ease-in-out ${
      isExpanded
      ? "fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-black text-white overflow-hidden rounded-none"
      : ""
    }`}
    >
    {/* Fullscreen backdrop */}
    ...
    {/* Title bar */}
    <div
    className={`flex items-center gap-2 border-b border-emerald-900/40 px-4 py-3 select-none text-sm w-full ${
      isExpanded ? "max-w-[1600px] mx-auto" : ""
    }`}
    >
    <ButtonDot
    color="bg-red-500"
    label={pinned ? "Unpin from top bar" : "Pin to top bar"}
    onClick={onTogglePin}
    />
    <ButtonDot
    color="bg-yellow-500"
    ...
    <ButtonDot
    color="bg-green-500"
    label={isExpanded ? "Exit fullscreen (Esc)" : "Enlarge for readability"}
    onClick={toggleExpand}
    />
    <div className="ml-3 font-mono text-xs text-emerald-300/80 truncate">
    $ {title}
    </div>
    </div>
    ...
    {/* ESC hint */}
    {isExpanded && (
      <div className="absolute top-5 right-8 text-base font-mono text-emerald-400/90 animate-pulse">
    ...
    {!minimized && (
      ...
      className={`transition-all duration-500 ease-in-out ${
        isExpanded
        ? "w-full h-full max-w-[1800px] px-[6vw] py-[6vh] overflow-y-auto text-[2.2rem] md:text-[2.6rem] leading-[1.9] tracking-wide"
        : "p-6 sm:p-8 text-[1rem] md:text-[1.1rem] leading-[1.8]"
      }`}
      >
      {children}
      </div>
...
function ProjectCard({ title, desc, stack, link }) {
  return (
    ...
    className="block rounded-2xl border border-emerald-900/40 bg-emerald-900/10 p-6 transition-all duration-300 ease-in-out hover:bg-white/5 hover:border-white/40 hover:shadow-[0_0_20px_rgba(255,255,255,0.15)]"
    >
    <div className="font-semibold text-xl text-emerald-200 group-hover:text-white">{title}</div>
